...
const char *Token::token_names[39] = {"ID", "LABEL", "NUM", "EOL", "ERR",
                                      "END", "PUSH", "EQ", "GT", "GE", "LT",
                                      "LE", "GOTO", "SKIP", "POP", "DUP",
                                      "SWAP", "ADD", "SUB", "MUL", "DIV",
                                      "STORE", "LOAD", "PRINT", "JMPZ", "JMPN",
                                      "AND", "OR", "NEG", "NOT", "HALT",
                                      "CALL", "MARK", "ENTER", "ALLOC",
                                      "STORER", "LOADR", "LOADA", "RETURN"};
...
Instruction::IType Token::tokenToIType(Token::Type tt) {
    ...
    switch (tt) {
        ...
        case (Token::DIV):
            ...
            break;
        case (Token::EQ):
            itype = Instruction::IEQ;
            break;
        case (Token::GT):
            itype = Instruction::IGT;
            break;
        case (Token::GE):
            itype = Instruction::IGE;
            break;
        case (Token::LT):
            itype = Instruction::ILT;
            break;
        case (Token::LE):
            itype = Instruction::ILE;
            break;
        case (Token::AND):
            itype = Instruction::IAND;
            break;
        case (Token::OR):
            itype = Instruction::IOR;
            break;
        case (Token::PRINT):
            itype = Instruction::IPRINT;
            break;
        case (Token::HALT):
            itype = Instruction::IHALT;
            break;
        case (Token::CALL):
            itype = Instruction::ICALL;
            break;
        case (Token::MARK):
            itype = Instruction::IMARK;
            break;

        case (Token::NEG):
            itype = Instruction::INEG;
            break;
        case (Token::NOT):
            itype = Instruction::INOT;
            break;
        ...
        case (Token::LOAD):
            ...
            break;
        case (Token::STORER):
            itype = Instruction::ISTORER;
            break;
        case (Token::LOADR):
            itype = Instruction::ILOADR;
            break;
        case (Token::PUSHA):
        ...
        case (Token::ENTER):
            itype = Instruction::IENTER;
            break;
        case (Token::ALLOC):
            itype = Instruction::IALLOC;
            break;
        case (Token::RETURN):
            itype = Instruction::IRETURN;
            break;
        ...
        case (Token::GOTO):
            ...
            break;
        case (Token::JMPZ):
            itype = Instruction::IJMPZ;
            break;
        case (Token::JMPN):
            itype = Instruction::IJMPN;
            break;
...
Instruction *Parser::parseInstruction() {
    ...
    if (match(Token::SKIP) || match(Token::POP) || match(Token::DUP) ||
        ...
        tipo = 0;
        ttype = previous->type;
    } else if (match(Token::PUSH) || match(Token::STORE) ||
               match(Token::LOAD) ||
               match(Token::ENTER) || match(Token::ALLOC) ||
               match(Token::STORER) || match(Token::LOADR) ||
               match(Token::RETURN)
            ) {
        tipo = 1;
        ttype = previous->type;
        if (!match(Token::NUM)) {
            cout << "Esperaba NUM despues de " << Token::token_names[ttype]
                 << endl;
            exit(0);
        }
        ...
        ttype = previous->type;
        if (!match(Token::ID)) {
            cout << "Esperaba ID despues de " << Token::token_names[ttype]
                 << endl;
            exit(0);
        }
