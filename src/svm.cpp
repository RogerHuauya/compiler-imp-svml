...
string snames[33] = {"push", "pop", "dup", "swap", "add", "sub", "mult", "div",
                     "goto", "eq", "gt", "ge", "lt", "le", "skip", "store",
                     "load", "print", "jmpz", "jmpn", "and", "or", "neg",
                     "not", "halt", "call", "mark", "storer", "loadr", "pusha",
                     "enter", "alloc", "return"};

Instruction::Instruction(string l, IType itype) : label(l), type(itype),
                                                  hasarg(false) {
}

Instruction::Instruction(string l, IType itype, int arg) : label(l),
                                                           type(itype),
                                                           hasarg(true),
                                                           argint(arg),
                                                           jmplabel("") {
...
Instruction::Instruction(string l, IType itype, string arg) : label(l),
                                                              type(itype),
                                                              hasarg(true),
                                                              jmplabel(arg) {
...
SVM::SVM(list<Instruction *> &sl) {
    ...
    for (int i = 0; i < instructions.size(); i++) {
        ...
        if (jl != "") {
            std::unordered_map<std::string, int>::const_iterator it = labels.find(
                    jl);
            if (it == labels.end()) {
    ...
    if (itype == Instruction::IPOP || itype == Instruction::IDUP ||
        ...
        switch (itype) {
            ...
            case (Instruction::ISTORE):
                if (stack_empty())
                    svm_error("Can't store from an empty stack");
        ...
        if (jump) pc = instr->argint; else pc++;
            ...
            (itype == Instruction::IADD || itype == Instruction::ISUB ||
             itype == Instruction::IMUL ||
             itype == Instruction::IDIV || itype == Instruction::IEQ ||
             itype == Instruction::IGT ||
             itype == Instruction::IGE || itype == Instruction::ILT ||
